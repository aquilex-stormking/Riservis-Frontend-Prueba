...
function examen($iMin, $iMax, &$aDatos){
	// Validar que los parámetros sean enteros y que iMin sea menor o igual a iMax
	if (!is_int($iMin) || !is_int($iMax) || $iMin > $iMax) {
		return false; // Retorna false si los parámetros no son válidos
	}

	$aDatos = []; // Inicializar el array
	...
	return true; // Retorna true si todo se ejecuta correctamente
